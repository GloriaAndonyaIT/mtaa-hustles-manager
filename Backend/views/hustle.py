...
def create_hustle():
    ...
    if not title or not type or not description or not date or not user_id:
        return jsonify({"error": "Title, type, description,date, and user_id are required"}), 400
    
    new_hustle = Hustle(title=title, type=type, description=description,date=date, user_id=user_id)
